...
const omit = <Obj extends {}, Key extends String & keyof Obj>(obj: Obj, omitKey: Key): Omit<Obj, Key> => {
    // deno-lint-ignore no-explicit-any
    return Object.fromEntries(Object.entries(obj).filter(([key, ]) => key !== omitKey)) as any
}
        ...
        up: compat.migrations.updateConfig(
          async (config, effects) => {
            ...
            if (hasLnd(config)) {
              return omit({
                ...config,
                nodes:  [{ type: 'lnd', name: 'Embassy LND', 'connection-settings': config.lnd }],
              }, 'lnd')
            }
            return config
        ...
        down: compat.migrations.updateConfig(
            async (config, effects) => {
                ...
                if (hasNodes(config)){
                  ...
                  const oldNodes = config.nodes.filter(isNodeShape).filter(x => x.type === 'lnd')
                  oldNodes.sort((left, right) =>
                    left["connection-settings"].type.localeCompare(right["connection-settings"].type)
                  );
